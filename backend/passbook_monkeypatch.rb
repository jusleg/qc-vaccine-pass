...
module Passbook
  class Signer
    ...
    def sign(data)
      wwdc  = OpenSSL::X509::Certificate.new wwdc_cert
      pk7   = OpenSSL::PKCS7.sign key_hash[:cert], key_hash[:key], data.to_s, [wwdc],
    ...
    def compute_cert
      @key_hash = {}
      if key
        @key_hash[:key]  = OpenSSL::PKey::RSA.new key, password
        @key_hash[:cert] = OpenSSL::X509::Certificate.new certificate
      else
        p12 = OpenSSL::PKCS12.new certificate, password
